"""Exported code from notebooks/raw_20260618/crossz.ipynb.
...
# %% [markdown] cell 1
# # TNG cross-redshift halo-dynamics analysis This notebook has two separated phases: first download/read cached TNG data and compute products; then reload the saved products and plot. Plotting cells do not download or recompute.
...
def plot_subhalo_orbitplane_overlay(
    ...
    *,
    ...
    ax.set_title(
        f"{SIM_NAME}: tracked subhalo {SID0} in fitted orbit plane\n"
        f"{method} shells; origin: MPB of z=0 host central",
        fontsize=16,
        color="black",
    )
...
#
# Fix:
#   when x_key == "Redshift", avoid duplicated Redshift columns.
#
# Usage:
...
def plot_pi_closure_table_evolution(
    ...
    *,
    ...
    ----------
    products : dict
        Result dictionary produced by the compute-products cell.
        Expected structure:
            products["methods"][method]["closure_df"]
            products["methods"][method]["track_df"]
    methods : sequence[str] or None
        Shell methods to plot. If None, use keys in products["methods"].
    closure_df : pandas.DataFrame or None
        If provided, use this table directly instead of collecting from products.
    path, OUTDIR : str/path or None
        Output directory.
    x_key : str
        Usually "Redshift".
    invert_redshift_axis : bool
        If True and x_key == "Redshift", high-z is on the left and low-z on the right.
    ...
    SID0 = int(products.get("SID0", products.get("sid", -1)))
    SNAP0 = int(products.get("SNAP0", products.get("snap0", 99)))
    SNAP_TRACK = list(products.get("SNAP_TRACK", []))
    ...
    def _drop_duplicate_columns(df):
        """
        Drop duplicated column names. This prevents pandas from returning a
        2-D DataFrame when df["Redshift"] is requested.
        """
    ...
    def _series(df, col, default=np.nan):
        """
        Return a 1-D Series even if the DataFrame has duplicated columns.
        """
        if col not in df.columns:
    ...
    if closure_df is None:
        ...
        if isinstance(method_blocks, dict):
            for method in methods:
                ...
                track = block.get("track_df", None)
                if isinstance(track, pd.DataFrame):
                    track_tables[str(method)] = track.copy()
    ...
    def _standardize_closure_df(df):
        ...
        if ca is not None:
            df["SnapA"] = pd.to_numeric(_series(df, ca), errors="coerce").astype("Int64")
        else:
            df["SnapA"] = pd.Series([pd.NA] * n, dtype="Int64")

        if cb is not None:
            df["SnapB"] = pd.to_numeric(_series(df, cb), errors="coerce").astype("Int64")
        else:
            df["SnapB"] = pd.Series([pd.NA] * n, dtype="Int64")
        ...
        # Subhalo ID.
        c = _choose_col(df, ["SubhaloID", "TargetSubfindID", "SubfindID", "sid", "SID"])
        if c is not None:
            df["SubhaloID"] = pd.to_numeric(_series(df, c), errors="coerce")
        else:
            df["SubhaloID"] = SID0
        ...
        for comp in COMPONENTS:
            ...
            direct = _numeric_series(df, f"Pi_direct_{comp}").to_numpy(dtype=float)
            ...
            # If affine Pi is missing but Omega and H are available, build it.
            missing_aff = ~np.isfinite(aff)
            can_make_aff = np.isfinite(Om) & np.isfinite(H)
            aff[missing_aff & can_make_aff] = Om[missing_aff & can_make_aff] + H[missing_aff & can_make_aff]
            df[f"Pi_aff_{comp}"] = aff

            # Residual definition used in the plots.
            df[f"Pi_resid_{comp}"] = direct - aff

            rel = np.full(n, np.nan, dtype=float)
            good = np.isfinite(direct) & np.isfinite(aff) & (np.abs(direct) > 0)
            rel[good] = (direct[good] - aff[good]) / np.abs(direct[good])
            df[f"rel_resid_{comp}"] = rel
            ...
            df[f"fOmega_abs_{comp}"] = fO
            df[f"fH_abs_{comp}"] = fH
            df[f"f_Omega_abs_{comp}"] = fO
            df[f"f_H_abs_{comp}"] = fH

        if x_key not in df.columns:
            raise KeyError(f"x_key={x_key!r} is not available in the closure table.")
    ...
    def _sort_df(df):
        df = _drop_duplicate_columns(df)
        x = _numeric_series(df, x_key).to_numpy(dtype=float)
    ...
    def _median_by_x(df, ycol):
        """
        Median across shells at fixed x. Crucially, this never selects
        [x_key, "Redshift", ycol] directly, so it cannot duplicate Redshift.
        """
        if ycol not in df.columns:
            return pd.DataFrame({x_key: [], "Redshift": [], ycol: []})
        ...
        good = np.isfinite(sub[x_key].to_numpy(dtype=float)) & np.isfinite(sub[ycol].to_numpy(dtype=float))
        ...
        med = (
            sub.groupby(x_key, as_index=False)
            .agg({
                ycol: "median",
                "Redshift": "median",
            })
    ...
    def _plot_line_with_redshift_points(ax, df, ycol, *, color, marker, label, norm, lw=1.75):
        ...
        good = np.isfinite(sub[x_key].to_numpy(dtype=float)) & np.isfinite(sub[ycol].to_numpy(dtype=float)) & np.isfinite(sub["Redshift"].to_numpy(dtype=float))
        sub = sub.loc[good].copy()
        ...
        for j, comp in enumerate(COMPONENTS):
            ...
            if aggregate:
                ...
                _plot_line_with_redshift_points(
                    ax,
                    _median_by_x(df, cdir),
                    cdir,
                    color=COLOR_DIRECT,
                    marker="o",
                    label=r"$\Pi^{\rm dI}$ median",
                    norm=norm,
                    lw=1.9,
                )
                ...
                _plot_line_with_redshift_points(
                    ax,
                    _median_by_x(df, caff),
                    caff,
                    color=COLOR_AFF,
                    marker="s",
                    label=r"$\Pi^{\Omega+\mathcal{H}}$ median",
                    norm=norm,
                    lw=1.9,
                )
            else:
    ...
    z = _numeric_series(closure_used, "Redshift").to_numpy(dtype=float)
    ...
    for method in methods:
        ...
        for shell_plot in sorted(dmethod["shell_plot"].dropna().unique().astype(int).tolist()):
            ds = dmethod.loc[
                dmethod["shell_plot"].astype(int) == int(shell_plot)
            ].copy()
    ...
    if show:
        ...
        for figdict in figs_by_method.values():
            for fig in figdict.values():
                plt.close(fig)

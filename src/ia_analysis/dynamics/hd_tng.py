...
def ckpc_h_to_physical_kpc(x, header):
    """
    Convert TNG comoving ckpc/h to physical kpc:
        x_phys[kpc] = x[ckpc/h] * a / h
    """
    arr = np.asarray(x, dtype=np.float64)
    out = arr * scale_factor_from_header(header) / hubble_from_header(header)
...
def cross_time_pattern_speed_for_subhalo(
    ...
    *,
    ...
    # Keep the local metadata variables consistent with the final catalogue
    # configuration.  This matters when the notebook/script supplies the TNG
    # API key and persistent cache through tng_catalog_kwargs rather than cfg.
    sim_name = str(cat_kwargs.get("sim_name", sim_name))
    api_key = cat_kwargs.get("api_key", api_key)

    tree = load_sublink_mpb(
        base_path,
        int(snap0),
        int(subhalo_id0),
        tng_catalog_kwargs=cat_kwargs,
    ...
    for _, row in track.iterrows():
        ...
        try:
            ...
            pdata_by_snap[snap]["redshift"] = float(header.get("Redshift", np.nan))
    ...
    for s0, s1 in zip(snaps[:-1], snaps[1:]):
        ...
        for ish, mi in enumerate(masks1):
            ...
            # Decompose the affine estimate into material-flow rotation and
            # Hessian/shear contributions:
            #     Pi_aff = Pi_Omega + Pi_H
            # with Pi_H_ij = eta_ij H_hat_ij.
            Omega_hat = np.asarray(fig1_aff.get("Omega_hat", np.full((3, 3), np.nan)), dtype=np.float64)
            H_hat = np.asarray(fig1_aff.get("H_hat", np.full((3, 3), np.nan)), dtype=np.float64)
            eta = np.asarray(fig1_aff.get("eta", np.full((3, 3), np.nan)), dtype=np.float64)

            Pi_Omega = Omega_hat * KM_S_PER_KPC_TO_GYR_INV
            Pi_H = eta * H_hat * KM_S_PER_KPC_TO_GYR_INV
            ...
            z_early = float(p0.get("redshift", np.nan))
            z_late = float(p1.get("redshift", np.nan))
            if np.isfinite(z_early) and np.isfinite(z_late):
                z_mid = 0.5 * (z_early + z_late)
            elif np.isfinite(z_late):
                z_mid = z_late
            else:
                z_mid = z_early

            row = {
                "shell_method": "binding_energy" if shell_method.startswith("binding") else "radial",
                "SnapA": int(s0),
                "SnapB": int(s1),
                "snap_early": int(s0),
                "snap_late": int(s1),
            ...
            for lab, i, j in [("01", 0, 1), ("02", 0, 2), ("12", 1, 2)]:
                row[f"Pi_fd_{lab}"] = float(Pi_fd[i, j])

                # Canonical names used by the plotting notebook.
                row[f"Pi_direct_{lab}"] = float(Pi_dir[i, j])
                row[f"Pi_aff_{lab}"] = float(Pi_aff[i, j])
                row[f"Pi_Omega_{lab}"] = float(Pi_Omega[i, j])
                row[f"Pi_H_{lab}"] = float(Pi_H[i, j])

                # Backward-compatible names used by older notebooks.
                row[f"Pi_direct_late_{lab}"] = float(Pi_dir[i, j])
                row[f"Pi_aff_late_{lab}"] = float(Pi_aff[i, j])

                row[f"Pi_resid_{lab}"] = float(Pi_dir[i, j] - Pi_aff[i, j])
                row[f"residual_{lab}"] = float(Pi_dir[i, j] - Pi_aff[i, j])

                denom = abs(Pi_Omega[i, j]) + abs(Pi_H[i, j])
                if np.isfinite(denom) and denom > 0:
                    row[f"fOmega_abs_{lab}"] = float(abs(Pi_Omega[i, j]) / denom)
                    row[f"fH_abs_{lab}"] = float(abs(Pi_H[i, j]) / denom)
                    row[f"f_Omega_abs_{lab}"] = row[f"fOmega_abs_{lab}"]
                    row[f"f_H_abs_{lab}"] = row[f"fH_abs_{lab}"]
                else:
                    row[f"fOmega_abs_{lab}"] = np.nan
                    row[f"fH_abs_{lab}"] = np.nan
                    row[f"f_Omega_abs_{lab}"] = np.nan
                    row[f"f_H_abs_{lab}"] = np.nan
